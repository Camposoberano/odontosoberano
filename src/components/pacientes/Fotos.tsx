...
export function Fotos({ pacienteId }: FotosProps) {
  const { pacientes } = usePacientes();
  const { fotos, loading, createFoto, deleteFoto } = useFotos(pacienteId);
  ...
  const handleSave = async () => {
    if (!pacienteId || !newFoto.imagem_base64 || !newFoto.titulo) {
      return;
    }
