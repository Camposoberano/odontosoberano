...
export function Radiografias({ pacienteId }: RadiografiasProps) {
  const { pacientes } = usePacientes();
  const { radiografias, loading, createRadiografia, updateRadiografia, deleteRadiografia } = useRadiografias(pacienteId);
  ...
  const handleSave = async () => {
    if (!pacienteId || !newRadiografia.imagem_base64 || !newRadiografia.titulo) {
      return;
    }
