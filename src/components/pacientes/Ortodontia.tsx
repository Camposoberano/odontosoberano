...
export function Ortodontia({ pacienteId }: OrtodontiaProps) {
  const { pacientes } = usePacientes();
  const { ortodontia, loading, createOrtodontia, updateOrtodontia } = useOrtodontia(pacienteId);
  ...
  const handleSave = async () => {
    ...
    try {
      if (ortodontia) {
        ...
        await createOrtodontia({ paciente_id: pacienteId, ...formData });
      }
    } catch (error) {
