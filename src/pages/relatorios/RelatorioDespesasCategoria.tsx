...
import { Printer, Tag, TrendingDown } from "lucide-react";
import { format, startOfMonth, endOfMonth, subMonths } from "date-fns";
import { motion } from "framer-motion";

type PeriodoKey = "mes_atual" | "mes_anterior" | "custom";

export default function RelatorioDespesasCategoria() {
  const { user } = useAuth();
  ...
  const { data: despesas = [], isLoading } = useQuery({
    queryKey: ["relatorio-despesas-categoria", user?.id, range.inicio, range.fim],
    enabled: !!user,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("contas_pagar")
        .select(`valor, data_vencimento, categorias_despesa(nome)`)
        .gte("data_vencimento", range.inicio)
        .lte("data_vencimento", range.fim);
      if (error) throw error;

      const mapa: Record<string, { categoria: string; qtd: number; total: number }> = {};
      for (const d of (data || [])) {
        const cat = (d as any).categorias_despesa?.nome ?? "Sem categoria";
        if (!mapa[cat]) mapa[cat] = { categoria: cat, qtd: 0, total: 0 };
        mapa[cat].qtd += 1;
  ...
  const totalGeral = despesas.reduce((s, d) => s + d.total, 0);
      ...
      <motion.div className="space-y-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground font-bold text-xs uppercase tracking-widest">
              Contas a pagar agrupadas por categoria
            </p>
          </div>
        ...
        <Card className="rounded-2xl border-2">
          <CardContent className="p-4 flex flex-wrap items-end gap-4">
            ...
            <Badge variant="destructive" className="h-9 px-4 text-base font-black ml-auto">
        ...
        <Card className="rounded-2xl border-2">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-black flex items-center gap-2">
              <TrendingDown className="w-5 h-5 text-red-500" /> Despesas por Categoria
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="p-8 text-center text-muted-foreground">Nenhuma despesa no período.</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 border-b">
                    <tr>
                      <th className="text-left px-4 py-3 font-black text-xs uppercase tracking-widest text-slate-500">Categoria</th>
                      <th className="text-right px-4 py-3 font-black text-xs uppercase tracking-widest text-slate-500">Qtd</th>
                      <th className="text-right px-4 py-3 font-black text-xs uppercase tracking-widest text-slate-500">Valor (R$)</th>
                    ...
                    {despesas.map((d, i) => (
                      <tr key={i} className="border-t border-slate-100 hover:bg-slate-50">
                        <td className="px-4 py-3 font-semibold text-slate-800 flex items-center gap-2">
                          <Tag className="w-4 h-4 text-slate-400" /> {d.categoria}
                        </td>
                        <td className="px-4 py-3 text-right text-slate-600">{d.qtd}</td>
                        <td className="px-4 py-3 text-right font-bold text-red-600">
                          {d.total.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                        </td>
                        <td className="px-4 py-3 text-right text-slate-500">
                          {totalGeral > 0 ? ((d.total / totalGeral) * 100).toFixed(1) : 0}%
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot className="bg-slate-50 border-t-2 font-black">
                    <tr>
                      ...
                      <td className="px-4 py-3 text-right">{despesas.reduce((s, d) => s + d.qtd, 0)}</td>
                      <td className="px-4 py-3 text-right text-red-600">{totalGeral.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</td>
