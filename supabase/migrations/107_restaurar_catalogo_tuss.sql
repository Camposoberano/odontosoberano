-- =============================================================================
-- Migration 107: Restaurar procedimentos_catalogo (TUSS/VRPO)
-- Foi removido por engano na migration 106 — apenas OS/protético deviam sair
-- =============================================================================
...
-- Restaurar FK em orcamento_itens
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'orcamento_itens_procedimento_id_fkey'
      AND table_name = 'orcamento_itens'
  ) THEN
    ALTER TABLE orcamento_itens
      ADD CONSTRAINT orcamento_itens_procedimento_id_fkey
      FOREIGN KEY (procedimento_id) REFERENCES procedimentos_catalogo(id) ON DELETE SET NULL;
  END IF;
END $$;
...
CREATE POLICY "catalogo_select" ON procedimentos_catalogo FOR SELECT TO authenticated USING (true);
CREATE POLICY "catalogo_insert" ON procedimentos_catalogo FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY "catalogo_update" ON procedimentos_catalogo FOR UPDATE TO authenticated USING (true);

-- Re-seed ~80 procedimentos TUSS
